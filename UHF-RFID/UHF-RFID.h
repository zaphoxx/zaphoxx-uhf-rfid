...
int generate_bytes(byte *payload, byte *command, unsigned int command_size);
...
void debug(String message, unsigned value);
...
**********************************************************************************************************************************/
int generate_bytes(byte *byte_array, byte *bit_array, unsigned int bit_array_size)
{
...
* !!! currently no crc check is performed after reading the data from tag !!!
**********************************************************************************************************************************/
bool read_data(byte *data, byte nwords)
{
  byte rxbuffer[MAX_PKT];
  byte data_bits[MAX_PKT * 8 / FM0_FACTOR]; // max databits is max pkt * 8bit / 2 (due to fm0 encoding)
  byte data_bytes[MAX_PKT];
  bool crc_ok = false;
  bool packet_received = false;
  int bytes_received = 0;
  int packetlength = ((nwords * 2) + RX_PADDING) * FM0_FACTOR;
  unsigned int nWordBits = nwords * 16;
  
  long int t1, t2;
  bool timeout = false;
  int max_timeout = 1 ; // timeout in sec

  // packet length is number of data bytes expected + 4 bytes rn16 + 4 bytes crc16 + 1 initial bit; times 2 due to fm0 encoding
  // this also means that the last 15 bits are just junk and need to be ignored / discarded

  // TODO: Implement option to exit after x tries - e.g. a timeout
  TX_UNIT.FillFifo();
  RX_UNIT.SpiWriteReg(CC1101_PKTLEN, packetlength);
  if (packetlength < -1)
  {
    TX_UNIT.FillFifo(); // fill TX FIFO / Send CW
    t1 = millis();
    RX_UNIT.SpiStrobe(CC1101_SRX);
    while(!RX_GDO0_STATE) 
    {
      TX_UNIT.FillFifo(); // keep TX FIFO Full until Sync Word is found
      t2 = millis();
      if ( ( ( t2 - t1 ) / 1000 ) > max_timeout )
      {
        timeout = true;
        debug("[READ] timeout ... ");
        break;
    ...
    if (!timeout)
    {
      while(RX_GDO0_STATE);                     // wait until packet is fully received
      TX_UNIT.FillFifo();
      RX_UNIT.SpiReadBurstReg(CC1101_RXFIFO, rxbuffer, packetlength); // read packet from RX FIFO
  ...
  else
  {
    ...
    // GDO2: asserst when RX is above threshold, deasserts if below threshold
    // GDO0: asserts when syncword is found, deasserts when packet complete
    
    TX_UNIT.FillFifo();
    debug("[!] packetlength > 64 bytes");
    t1 = millis();
    RX_UNIT.SpiStrobe(CC1101_SRX);
    while(!RX_GDO0_STATE) 
    {  
    ...
    if (!timeout)
    {    
      while (bytes_received < packetlength)
      {
        if (( packetlength - bytes_received ) > FIFO)
        {
          while(!RX_GDO2_STATE);
          rxbuffer[bytes_received] = RX_UNIT.SpiReadReg(CC1101_RXFIFO);
          bytes_received++;
          TX_UNIT.FillFifo();
        }
        else
  ...
  if (packet_received)
  {
    ...
    debug("[RX] Buffer: ",rxbuffer, packetlength);
    //debug(data_bits, (nwords * 16 + 1 + 16 + 16));
    if (data_bits[0] == 0)
    {
      unsigned int data_size = generate_bytes(data_bytes, data_bits + 1, nWordBits);
      memcpy(data, data_bytes, data_size);
      TX_UNIT.FillFifo();

      // Do CRC Check //
      uint8_t rcrc16[2];
      generate_bytes(rcrc16, data_bits + 1 + nWordBits + 16 , 16 );
      debug("[RX] wordbit count: ",nWordBits);
      debug("[RX] received CRC16: ", rcrc16,2);
      byte crc16_bits[16];
      uint16_t ccrc16 = crc16B(crc16_bits, data_bits, nWordBits + 16 + 1);
    ...
    {
      int data_size = generate_bytes(data_bytes, data_bits + 1, nWordBits);
      error("[READ] ERROR. Code: ", &data_bytes[0], 1);
      error("[READ] " + ERRORCODE[data_bytes[0]]);
    }
  }
  return (packet_received && crc_ok);
}
/********************************************************************************************************************************
...
bool search_write_ack()
{
  ...
  bool found = false;
  bool crc_ok = false;
  byte rxbuffer[packetsize]; // ( 16 RN16 + 16 CRC + 1 ) * 2 Bits
  byte databuffer[packetsize * 4];
  bool timeout = false;
  long int t1, t2;
  byte max_timeout = 1;
  
  byte packetlength = packetsize ;
  RX_UNIT.SpiWriteReg(CC1101_PKTLEN, packetlength);
  ...
  if (!timeout)
  {
    debug("[WRITE] Found SYNC");
    if(RX_GDO0_STATE) found = true;
    while(RX_GDO0_STATE);                     // wait until packet is fully received
    TX_UNIT.FillFifo();
    RX_UNIT.SpiReadBurstReg(CC1101_RXFIFO, rxbuffer, packetlength); // read packet from RX FIFO
    debug("RXBUFFER",rxbuffer,packetlength);
  }  
  /* original code 
...
void debug(String message, unsigned int value)
{
  Serial.println("#DEBUG");
