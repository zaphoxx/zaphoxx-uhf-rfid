...
def main():
    ...
    while line != b"#END":
      ...
      if (line == b"#PRETEAR" and args.tears_flag):
        ...
        print(Fore.CYAN,end='')
        print(F"{int.from_bytes(n_writes,'little')} - {int.from_bytes(n_writes,'little') * 12.5} µs: "+" ".join(format(x, "02x") for x in buffer))
      ...
      # general read data results
        ...
        print("#"*58)
        buffer = ser.read(args.n_words * 2)
        for i in range(0,args.n_words,8):
          print(Fore.CYAN + F"{(i*2)+args.block_addr:#08x}: " + " ".join(format(x, "02x") for x in buffer[(i*2):(i*2)+16]),end=" ")
          print();
...
def process_args(parser):
    parser.add_argument('-p', dest='port', help='serial port e.g. /dev/ttyACM0', default='/dev/ttyACM0')
    parser.add_argument('-b', dest='baudrate', type=int, help='baudrate of serial com', default=115200)
    parser.add_argument('-t', dest='timeout', help='serial read timeout', type=int, default=3)
    parser.add_argument('-r', dest='repetitions', help='repetitions to run before terminating',type=int, default=100)
    ...
    parser.add_argument('-v', dest='verbose', help='additional output', action='store_true')
    ...
    parser.add_argument('-n',dest = 'n_words', help='read n words from mem block',type=int, default = 1) 
    # maximum value allowed is 62 so the maximum packet to read does not exceed 255 bytes; ((62 * 4) + 4 + 1 < 255)
    ...
    action.add_argument('-reqrn', dest = 'reqrn_flag', action='store_true', help='repetitive request rn16 ... ')
